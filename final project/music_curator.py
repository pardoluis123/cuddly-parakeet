...
def run_music_picker(current_state):
    ...
    st.subheader("Lets get some awesome music suggestions going!!!!!!! WOHOOOOO :)))))))) ")
...
def create_OpenAI_embeddings(df):
    ...
    combined_texts = df.apply(lambda row: ' '.join([str(val) for val in row]), axis=1).tolist()
...
def create_regular_embeddings(dataframe):
    ...
    release_dates_raw=dataframe['release_date'].to_list()
    release_dates_raw=[float(''.join(i.split('-'))) for i in release_dates_raw]
    dataframe['release_date']=release_dates_raw
